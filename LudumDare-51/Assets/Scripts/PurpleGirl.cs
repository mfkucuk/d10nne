...
public class PurpleGirl : MonoBehaviour
{
    ...
    [SerializeField]
    private OrangeGirl _orangeGirl;
    ...
    public bool purpleISdead { get; set; } = false;
    ...
    private void Update()
    {
        if (!purpleISdead && !_orangeGirl.orangeISDead)
        {
            ...
            else
            {
                if (!Timer10.paused)
                {
                    ...
                    if (followTime <= 0)
                    {
                        _rigidbody.position = _orangeGirl.positions[posIndex++];
                    }
                }
            }
        }
    ...
    public IEnumerator Rewind()
    {
        if (!isGhost)
        {
            ...
            transform.DOMove(_orangeGirl.transform.position, 1f);
